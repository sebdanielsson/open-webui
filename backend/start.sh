...
if [[ -z "${WEBUI_SECRET_KEY:-}" && -z "${WEBUI_JWT_SECRET_KEY:-}" ]]; then
  ...
  # Regenerate when missing, empty or not a regular file, never when merely
  # unreadable: such a key may belong to another UID, and it also encrypts OAuth
  # sessions and valves.
  if ! [[ -f "$KEY_FILE" && -s "$KEY_FILE" ]]; then
    ...
    # 0640 key, 0750 parents: a volume remounted under a different arbitrary
    # UID keeps group 0, and nothing is exposed to the rest of the image.
    (
      umask 027
